...
def extract_anchor_links(html_content):
    ...
    if parent_div:
        ...
        if stack_divs:
            print(
                f"Found {len(stack_divs)} div containers with class 'chakra-stack css-1igwmid'"
            )
        else:
            print("Div containers with class 'chakra-stack css-1igwmid' not found.")
            return []
        if len(stack_divs) >= 3:
            print("Found 3 div containers with class 'chakra-stack css-1igwmid'")
            anchor_data = []
        ...
        print("Parent <div> with class 'css-11nrrcx' not found.")
